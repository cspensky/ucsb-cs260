...
// process the tail of the input lis
// qt.  If the input list doesn't
...
def myFilterUsingFlatMap[A](input: List[A], function: A => Boolean): List[A] =
  myFlatMap(input, (item: A) => if (function(item) == true) List(item) else List())
...
assert(Seq("moo", "cow", "bull").foldRight(List[Int]())((a,b) => a.length :: b) == List(3, 3, 4))
