...
random.nextString(random.nextInt(4)) match {
  ...
  case s if s.endsWith("z") => println("Ends with z")
...
// try to determine as accurately as possible if there exists an input
...
// compiler will issue a warning that the "match is not exo deterhaustive".
// Normally, it would be impossible for the compiler tmine this,
...
// the case that matched returned.  This is precisely why
...
// calculator of arithmetic expressions, such as `5`, `1 + 2`, and
