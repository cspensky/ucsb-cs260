...
object Helpers {
  ...
  def initstate(p: Program): State = {
    ...
    val addr = Address()
    // Define our fields
    val flds = θ(cn)._1.foldLeft(ListMap.empty[Var, Value]) { (m, kv) => m+ (kv._1 -> get_default(kv._2)) }
  ...
  def call(x:Var, addr:Address, heap:Heap, mn:MethodName, values:Seq[Value], locals:Locals, kont:Seq[Kont] ): State = {
    ...
    val method_flds = methods(mn).params.foldLeft(ListMap.empty[Var, Value]) { (m, decl) => m + (decl.x -> get_default(decl.τ)) }
    ...
    val new_locals = method_flds.zipWithIndex.foldLeft(ListMap.empty[Var,Value]) {
      (m,e) =>
  ...
  def construct(x:Var, cn:ClassName, values:Seq[Value], locals:Locals, heap:Heap, kont:Seq[Kont] ): State = {
    ...
    val method_flds = methods(cn).params.foldLeft(ListMap.empty[Var, Value]) { (m, decl) => m + (decl.x -> get_default(decl.τ)) }
